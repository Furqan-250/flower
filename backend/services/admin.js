...
var classesModel = require("../models/classes");
...
var addClasses = (req, res, next) => {
  ...
  if (creator == null) {
    ...
    var name = req.body.name;
    classesModel.findOne({ name: name }).then((classes) => {
      // classes already exists
      if (classes) {
        res.json({
          success: false,
          message: "Classes is already exists!",
        });
      } else {
        //add subject to database
        var newclasses = new classesModel({
          name: name,
          status: true,
          createdBy: creator._id,
        });
